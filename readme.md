...
- [+] Each opponent has its own attack/defense profile.
- [+] The demo, for example, has 3 opponents - a spider that attacks two zones and blocks one, a troll that attacks one zone and blocks three, and so on. Your opponent pool must include at least 2 opponents with different profiles.
- [+] The opponent's choices each turn are random within its profile.
- [+] Random zones must not repeat within the same turn (e.g. the spider can't attack the legs twice in one turn).
...
## Character page
- [+] avatar, name, win/loss record, ability to change the avatar.

## Home page
- [+] start button that creates a new battle.

## Registration screen
- [+] single input for the player's name.